const slider = () => {
	...
	const prevSlide = (elemsArr, index, strClass) => {
		elemsArr[index].classList.remove(strClass) 							//удаляем у действующего слайда класс
	}

	const nextSlide = (elemsArr, index, strClass) => {
		elemsArr[index].classList.add(strClass)  							 //добавляем новому слайду класс
	}
